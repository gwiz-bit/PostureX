...
|---|---|---|
| **I — Việc cần làm & trạng thái** | Việc ưu tiên, tình trạng commit, tiến độ so với plan | **Mở máy là đọc phần này trước** |
| **II — Chạy hệ thống** | Lệnh chạy backend/app, bẫy emulator, chạy trên điện thoại | Khi cần chạy lại hệ thống |
| **III — Tồn đọng & bài học** | Bug chưa sửa, nợ kỹ thuật, bẫy đã trả giá để biết | Trước khi sửa gì đó |
| **IV — Nhật ký** | Lịch sử làm việc theo ngày | Khi cần biết "cái này làm hồi nào, vì sao" |

> 📌 **Quy ước: NHẬT KÝ xếp cũ ở trên, MỚI NHẤT Ở DƯỚI CÙNG.**
> Viết thêm đợt mới thì **thêm vào cuối Phần IV**, đừng chèn vào giữa.

---

# 🔥 PHẦN I — VIỆC CẦN LÀM & TRẠNG THÁI

## ▶️ LẦN SAU MỞ MÁY LÀM GÌ

### ✅ Trạng thái chốt (14/07/2026)

**Code đã an toàn.** Nhánh `hiepga` = **11 commit**, đã push hết lên GitHub, không còn gì nằm trên máy.
`main` **chưa bị đụng**. **Chưa tạo Pull Request** (cố ý — BE-13 còn thiếu phần Flutter của FCM).

Backend: **77 test pass**. BE-13 ~90%, BE-14 ~95% (**MoMo sandbox chạy thật**).

Chi tiết 11 commit: xem mục *"TÌNH TRẠNG COMMIT"* ngay dưới.
...
> Việc này **quan trọng hơn toàn bộ code còn lại**. Đã treo 3 ngày.
...
| **Thu tiền THẬT (PayOS)** | Đăng ký `payos.vn` bằng CCCD → 3 khoá | Hiện đang dùng **MoMo sandbox** (tiền giả). Xem mục *THU TIỀN THẬT*. Code gần như không đổi. |
...
3. **Test xanh ≠ chạy đúng.** Test SQLite đều pass nhưng vẫn có bug chỉ MySQL mới lộ ra (xem mục "Bug chỉ MySQL mới lộ ra"). SQLite không có CHECK constraint và ràng buộc thật của MySQL. **Việc gì quan trọng thì phải chạy thử trên MySQL.**
...
- **Đừng ghi "đã có tự động gia hạn"** — cổng thanh toán không trừ tiền định kỳ được. Cờ `AutoRenew` chỉ để *nhắc* gia hạn.
- **Đừng ghi "đã tích hợp IAP"** — dùng **MoMo**, không phải in-app purchase. (Và bán nội dung số qua cổng ngoài là **vi phạm chính sách Google Play** nếu lên store.)
- **Đừng ghi "đã thanh toán được"** — đang chạy **MoMo SANDBOX** (tiền giả). Ghi trung thực: *"tích hợp cổng MoMo, môi trường sandbox"*.
...
### ⚠️ Bảng `device_tokens` biến mất sau khi DB bị nạp lại
...
⚠️ **Lưu ý cho báo cáo:** plan ghi công cụ thanh toán là *"Stripe / IAP"*. Thực tế đang dùng **MoMo (sandbox)**. Google Play **bắt buộc** dùng Google Play Billing cho nội dung số — bán Premium qua MoMo/VNPay là **vi phạm chính sách nếu lên store**. Đồ án thì không sao, nhưng **đừng ghi là "đã tích hợp IAP"**.

---

## 📦 TÌNH TRẠNG COMMIT — 14/07/2026

Nhánh `hiepga`: **11 commit**, **đã push hết** lên GitHub. Không còn gì nằm trên máy.
`main` **chưa bị đụng tới**. **Chưa tạo Pull Request** (cố ý — BE-13 còn thiếu phần Flutter của FCM).

```
main   ── c4cc5f1 (Merge branch 'Viet')
              hiepga         94d8c34 → 9c56e81 → d0ec4d7 → b60459c → 26ff29b
               → 17951e9 → a6d3a5a → 41d5e09 → 900ebe2 → 96d0de3 → 87cde3d
                                                                    ↑ mới nhất
```

| Commit | Nội dung |
|---|---|
| `87cde3d` | **Đổi VNPay → MoMo sandbox** (Đợt 4) |
| `96d0de3` | Cập nhật tài liệu |
| `900ebe2` | **Scheduler 3 job + BE-14 quản lý gói** (Đợt 3) |
| `41d5e09` | **FCM push — backend** (Đợt 3) |
| `a6d3a5a` | Cập nhật tài liệu |
| `17951e9` | **Fix BUG-1..BUG-4** (hết hạn gói, quyền Premium, thông báo, test) |
| `26ff29b` | Xoá prototype `posturex_flutter` (133 file) |
| `b60459c` | Cập nhật tài liệu |
| `d0ec4d7` | **BE-13 thông báo + BE-14 thanh toán** (bản VNPay đầu tiên) |
| `9c56e81` | Fix Kotlin `jvmTarget = 17` + API host (không có thì **không build được**) |
| `94d8c34` | **Fix `websocket.disconnect` trong `/ws/analyze`** — *tách riêng có chủ ý*, vì đây là **code của thành viên khác**. Gửi họ đúng mã commit này để cherry-pick. |

### Cách kiểm chứng trước khi push (làm mỗi lần)

**Không tin file trên đĩa** — giải nén đúng trạng thái đã commit ra thư mục tạm rồi chạy từ đó:

```bash
git archive HEAD | tar -x -C /tmp/check
cd /tmp/check/lib/backend
python -c "import app.main"   # quen mot file la backend CRASH ngay khi import
pytest                        # phai thay 77 passed
```

Và kiểm tra bí mật **chưa từng** lọt vào lịch sử git (không chỉ commit mới nhất):

```bash
git log --all --pretty=format: --name-only | sort -u | grep -x "lib/backend/.env"
```

### Cố ý KHÔNG commit

`linux/`, `macos/`, `windows/` — git báo "modified" nhưng nội dung không đổi, chỉ là nhiễu
line-ending LF→CRLF. Commit vào chỉ tạo conflict vô nghĩa cho teammate.

# ⚙️ PHẦN II — CHẠY HỆ THỐNG (tra cứu)
...
# 📓 PHẦN IV — NHẬT KÝ (cũ ở trên, MỚI NHẤT Ở DƯỚI CÙNG)
