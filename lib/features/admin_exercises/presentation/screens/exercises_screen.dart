...
import '../../admin_exercises_module.dart';
...
class _ExercisesScreenState extends State<ExercisesScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final exercises = await AdminExercisesModule.getAdminExercises()();
      if (!mounted) return;
  ...
  Future<void> _toggleActive(AdminExercise ex) async {
    ...
    if (!ok) return;
    try {
      await AdminExercisesModule.updateAdminExercise()(ex.id, isActive: targetActive);
      if (mounted) _load();
    } on AppFailure catch (e) {
  ...
  Future<void> _delete(AdminExercise ex) async {
    ...
    if (!ok) return;
    try {
      await AdminExercisesModule.deleteAdminExercise()(ex.id);
      if (!mounted) return;
  ...
  Future<void> _pickAndUploadVideo(AdminExercise ex) async {
    ...
    final validationError = AdminExercisesModule.validateExerciseVideoFile(
      path: picked.path,
      sizeBytes: size,
    );
    if (validationError != null) {
      if (mounted) showToast(context, validationError);
      return;
    }

    setState(() => _uploadingVideoForId = ex.id);
    try {
      await AdminExercisesModule.uploadAdminExerciseVideo()(exerciseId: ex.id, file: file);
      if (mounted) {
  ...
  Future<void> _removeVideo(AdminExercise ex) async {
    ...
    setState(() => _uploadingVideoForId = ex.id);
    try {
      await AdminExercisesModule.deleteAdminExerciseVideo()(ex.id);
      if (mounted) {
