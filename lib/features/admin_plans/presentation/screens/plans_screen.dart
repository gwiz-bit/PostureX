...
import '../../admin_plans_module.dart';
...
class _PlansScreenState extends State<PlansScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final plans = await AdminPlansModule.getAdminPlans()();
      if (!mounted) return;
  ...
  Future<void> _toggleActive(AdminPlan plan) async {
    ...
    if (!ok) return;
    try {
      await AdminPlansModule.updateAdminPlan()(plan.id, isActive: targetActive);
      if (mounted) _load();
    } on AppFailure catch (e) {
...
class _PlanFormSheetState extends State<_PlanFormSheet> {
  ...
  Future<void> _save() async {
    ...
    final validationError =
        AdminPlansModule.validatePlanForm(name: name, priceText: _price.text);
    ...
    try {
      if (_isEdit) {
        await AdminPlansModule.updateAdminPlan()(
          widget.plan!.id,
          name: name,
          priceMonthly: price,
        ...
        await AdminPlansModule.createAdminPlan()(
          name: name,
          priceMonthly: price,
          currency: _currency.text.trim().isEmpty ? 'VND' : _currency.text.trim(),
          features: _features.text.trim().isEmpty ? null : _features.text.trim(),
